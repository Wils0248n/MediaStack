...
class QueryHandler:
    ...
    def __handle_album_request(self, request: str) -> bytes:
        ...
        if not self.__media_manager.is_album(album_name):
            return None
        if len(album_query) == 2:
            current_index = int(request.split('/')[2])
            media = self.__media_manager.find_album(album_name).media_list[current_index]
            html_code = self.__web_generator.generate_album_media_page(media, current_index)
        elif len(album_query) == 1:
            album = self.__media_manager.find_album(album_name)
            html_code = self.__web_generator.generate_album_page(album)
    ...
    def __handle_all_media_request(self) -> bytes:
        return bytes(self.__web_generator.generate_all_page(self.__media_manager.get_all_media()), self.__encoding)
    ...
    def __handle_search_all_request(self, request: str) -> bytes:
        ...
        return bytes(self.__web_generator.generate_all_page(media, search_query), self.__encoding)
